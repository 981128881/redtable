<template>
	...
	<view class="card">
		...
		<view class="date_week">
			<view class="week_td" v-for="(item,index) in week" :key="index">{{item}}</view>
		</view>
		<swiper :style="{height: '480rpx'}" :current="current" circular @change="change_date">
			<swiper-item>
				<view class="date_week">
					<view class="week_td" :class="select_list.includes(vo.date) && vo.type == 'month' ? 'select' : ''
						" @tap="select(vo, index, week_list_prev)" @longtap="selectMore(vo, index)" v-for="(vo,index) in week_list_prev"
						:key="index">
						<view class="num"
							:class="[vo.today && vo.type == 'month' ? 'today': '',vo.type == 'month' ? 'month' : 'disabled']">
							{{vo.day}}</view>
			...
			<swiper-item>
				<view class="date_week">
					<view class="week_td" :class="select_list.includes(vo.date) && vo.type == 'month' ? 'select' : ''"
						@tap="select(vo, index, week_list)" @longtap="selectMore(vo, index)"
						v-for="(vo,index) in week_list" :key="index">
						<view class="num"
							:class="[vo.today && vo.type == 'month' ? 'today': '',vo.type == 'month' ? 'month' : 'disabled']">
							{{vo.day}}</view>
						<view v-show="vo.dot && vo.type == 'month'" class="dot"></view>
			...
			<swiper-item>
				<view class="date_week">
					<view class="week_td" :class="select_list.includes(vo.date) && vo.type == 'month' ? 'select' : ''"
						@tap="select(vo, index, week_list_next)" @longtap="selectMore(vo, index)"
						v-for="(vo,index) in week_list_next" :key="index">
						<view class="num"
							:class="[vo.today && vo.type == 'month' ? 'today': '',vo.type == 'month' ? 'month' : 'disabled']">
							{{vo.day}}</view>
						<view v-show="vo.dot && vo.type == 'month'" class="dot"></view>
		...
		methods: {
			...
			get_date(value = '', type = 'same') {
				...
				for (let i = prev_date_days - start_week + 1; i <= prev_date_days; i++) {
					let currtDate =
						`${prev_date.getFullYear()}-${prev_date.getMonth()+1 > 9 ? prev_date.getMonth()+1 : '0' + (prev_date.getMonth()+1)}-${i > 9 ? i : '0' + i}`
					date_arrs.push({
						day: i,
						type: 'prev',
				...
				for (let i = 1; i <= days; i++) {
					let currtDate = `${nowYear}-${nowMonth > 9 ? nowMonth : '0' + nowMonth}-${i > 9 ? i : '0' + i}`
					date_arrs.push({
						day: i,
						type: 'month',
						today: i == new Date().getDate() ? true : false,
				...
				for (let i = 1; i <= count_days - date_arrs_length; i++) {
					let currtDate =
						`${next_date.getFullYear()}-${next_date.getMonth()+1 > 9 ? next_date.getMonth()+1 : '0' + (next_date.getMonth()+1)}-${i > 9 ? i : '0' + i}`
					date_arrs.push({
						day: i,
						type: 'next',
						today: i == new Date().getDate() ? true : false,
			...
			// 滑动切换
			change_date(e) {
				let primary_current = this.current; // 之前显示的swiper-item
			...
			doc_list_update(week_list = 'week_list') {
				...
				this[week_list].forEach((vo, key) => {
					...
					// 按星期的范围除去特定日期标记
					if (this.dot_week_lists.indexOf(vo.dayWeek) > -1 && this.dot_remove_lists.indexOf(vo.date) == -1) {
						vo.dot = true;
					} else {
						vo.dot = false;
					}
